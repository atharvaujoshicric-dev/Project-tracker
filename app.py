...
if not st.session_state.logged_in:
    ...
    if choice == "My Tasks":
        ...
        if my_projects:
            ...
            f_stat = col_f2.radio("Status", ["Pending", "Completed"], horizontal=True)

            # Add Task Section
            with st.expander("➕ Add New Task to this Project"):
            ...
            # Display Tasks
            st.subheader(f"Tasks for {sel_proj_name}")
            ...
            if tasks:
                ...
                sel_task = st.selectbox("Update Status for Task ID:", df["ID"])
                if st.button("Mark as Complete" if f_stat == "Pending" else "Re-open Task"):
                    new_status = "Completed" if f_stat == "Pending" else "Pending"
                    run_query("UPDATE tasks SET status=? WHERE task_id=?", (new_status, sel_task))
                    st.rerun()
            else: st.info("No tasks found.")
        else: st.warning("No projects assigned to you yet. Please contact Admin.")

    # --- ADMIN CONTROL ---
    elif choice == "Admin Control":
        st.header("Admin Dashboard")
        t1, t2 = st.tabs(["Manage Users", "Manage Projects"])
        ...
        with t1:
            ...
            with col2:
                ...
                du = st.selectbox("Select User", [u[0] for u in run_query("SELECT username FROM users", fetch=True)])
        ...
        with t2:
            ...
            pn = st.text_input("Project Name (e.g., Raju Kalate Wakad)")
            po = st.selectbox("Assign to User", [u[0] for u in run_query("SELECT username FROM users", fetch=True)])
